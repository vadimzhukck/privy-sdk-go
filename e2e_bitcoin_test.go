...
// ============================================
// Bitcoin RawSign E2E Tests
// ============================================

func TestE2E_Bitcoin_RawSign(t *testing.T) {
	...
	resp, err := client.Wallets().Bitcoin().RawSign(ctx, wallet.ID, hash)
	if err != nil {
...
func TestE2E_Bitcoin_RawSignBytes(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()
	...
	resp, err := client.Wallets().Bitcoin().RawSignBytes(ctx, wallet.ID, "48656c6c6f", "hex", "sha256")
	if err != nil {
...
func TestE2E_Bitcoin_RawSignWithNonExistentWallet(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()

	ctx := context.Background()

	_, err := client.Wallets().Bitcoin().RawSign(ctx, "non-existent-wallet-id", "0xabcdef")
	if err == nil {
