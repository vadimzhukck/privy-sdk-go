...
// ============================================
// Sui RawSign E2E Tests
// ============================================

func TestE2E_Sui_RawSign(t *testing.T) {
	...
	resp, err := client.Wallets().Sui().RawSign(ctx, wallet.ID, hash)
	if err != nil {
...
func TestE2E_Sui_RawSignBytes(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()
	...
	resp, err := client.Wallets().Sui().RawSignBytes(ctx, wallet.ID, "48656c6c6f", "hex", "sha256")
	if err != nil {
...
func TestE2E_Sui_RawSignWithNonExistentWallet(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()

	ctx := context.Background()

	_, err := client.Wallets().Sui().RawSign(ctx, "non-existent-wallet-id", "0xabcdef")
	if err == nil {
