...
// ============================================
// NEAR RawSign E2E Tests
// ============================================

func TestE2E_Near_RawSign(t *testing.T) {
	...
	resp, err := client.Wallets().Near().RawSign(ctx, wallet.ID, hash)
	if err != nil {
...
func TestE2E_Near_RawSignBytes(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()
	...
	resp, err := client.Wallets().Near().RawSignBytes(ctx, wallet.ID, "48656c6c6f", "hex", "sha256")
	if err != nil {
...
func TestE2E_Near_RawSignWithNonExistentWallet(t *testing.T) {
	client, server, _ := setupTestServer(t)
	defer server.Close()

	ctx := context.Background()

	_, err := client.Wallets().Near().RawSign(ctx, "non-existent-wallet-id", "0xabcdef")
	if err == nil {
